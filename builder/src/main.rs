...
pub fn create_disk_images(kernel_binary_path: &Path) -> PathBuf {
    let bootloader_manifest_path = PathBuf::from(
        "/home/amber/.cargo/registry/src/github.com-1ecc6299db9ec823/bootloader-0.10.12/Cargo.toml",
    );
    let kernel_manifest_path = locate_cargo_manifest::locate_manifest().unwrap();

    let toml =
        Manifest::<toml::Value>::from_path_with_metadata(kernel_manifest_path.clone()).unwrap();

    println!("{:#?}", toml);

    // let kernel_package = toml.workspace.unwrap().metadata.unwrap().get("kernel-builder").unwrap().get("package-name").unwrap().as_str().unwrap();
